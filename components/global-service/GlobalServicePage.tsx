...
export function GlobalServicePage({ data }: Props) {
  ...
  return (
    <>
      {/* HERO */}
      <section className={`relative overflow-hidden ${bgLight} py-14 md:py-20 border-b border-gray-200`}>
        ...
        <div className="container relative mx-auto px-4">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <span className={`inline-flex items-center gap-2 ${bgAccent} text-white text-xs font-bold px-4 py-2 rounded-full uppercase tracking-wide mb-5 shadow-sm`}>
                {theme.badge}
              </span>
              <h1 className="text-4xl md:text-5xl font-extrabold text-gray-950 mb-4 leading-tight tracking-tight">
                {data.hero_heading || data.service_name}
              </h1>
              <p className="text-lg md:text-xl text-gray-700 mb-4 leading-relaxed">
                {data.hero_subheading}
              </p>
              {data.hero_badge_text && (
                <p className={`inline-flex items-center gap-2 text-sm ${accentText} bg-white border ${borderClr} rounded-xl px-4 py-3 mb-7 shadow-sm`}>
                  <MapPin className="w-4 h-4" /> {data.hero_badge_text}
                </p>
              )}

              <div className="grid grid-cols-3 gap-3 max-w-lg mb-8">
                <Stat value="24/7" label="Support" accentText={accentText} />
                <Stat value="30–60" label="Min Arrival" accentText="text-gray-900" />
                <Stat value="4.8★" label="Trusted" accentText="text-green-600" />
              </div>

              <div className="flex flex-wrap gap-3">
                <Link href="#contact" className={`${bgAccent} ${btnHover} text-white px-8 py-3 rounded-xl font-bold transition-colors shadow-md`}>
                  Book {data.service_name}
                </Link>
                <a href={`tel:${MAIN_PHONE_DISPLAY}`} className="bg-white border-2 border-gray-200 text-gray-900 px-6 py-3 rounded-xl font-bold hover:border-gray-400 transition-colors flex items-center gap-2 shadow-sm">
                  <Phone className="w-4 h-4" /> {MAIN_PHONE_DISPLAY}
                </a>
                <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=Hi, I need ${data.service_name} service`} target="_blank" rel="noopener noreferrer" className="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-xl font-bold transition-colors shadow-sm">
                  WhatsApp
                </a>
              </div>
            </div>
            ...
            <div className="hidden md:block relative">
              <div className={`rounded-[2rem] overflow-hidden shadow-2xl h-[460px] w-full relative ring-8 ${ringClr}`}>
                <Image
                  src={data.hero_image_url || heroImage}
                  alt={data.hero_image_alt || `${data.service_name} service`}
                  fill
              ...
              {data.schema_aggregate_rating > 0 && (
                <div className="absolute -bottom-5 -left-5 bg-white text-gray-950 p-5 rounded-2xl shadow-xl border border-gray-100">
                  <p className="text-3xl font-extrabold">{data.schema_aggregate_rating}/5</p>
                  <p className="text-xs font-semibold text-gray-500">{data.schema_review_count}+ Reviews</p>
                </div>
              )}
            </div>
          </div>
